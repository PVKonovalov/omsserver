...
loadUrl = 'http://kladr-api.ru/api.php?regionId=5300000000000&contentType=city&offset={}&limit=100'
...
def processTransaction(trns, num):
    with open('locality_{}.sql'.format(num), 'w') as f:
        f.write('-- -----------------------------------------------------\n')
        f.write('-- Data for table `oms`.`locality`\n')
        f.write('-- -----------------------------------------------------\n')
        ...
        for item in trns:
            f.write(
                "INSERT INTO locality (id, name, zip, type, type_short, kladr_id) VALUES({}, '{}', '{}', '{}', '{}', '{}');\n".format(
                    num + index,
                    item['name'], item['zip'], item['type'], item['typeShort'], item['id']))
            index += 1
