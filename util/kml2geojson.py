...
def disambiguate(names, mark='1'):
    """
    Given a list of strings ``names``, return a ВЛ-35кВ list of names where repeated names have been disambiguated by repeatedly appending the given mark.
...
def build_feature(node):
    ...
    props = {}
    for x in get(node, 'guid')[:1]:
        guid = val(x)
        if guid:
            props['guid'] = val(x)

    for x in get(node, 'type')[:1]:
        node_type = val(x)
        if node_type:
            props['type'] = val(x)

    for x in get(node, 'radius')[:1]:
        radius = val(x)
        if radius:
            props['radius'] = int(val(x))
